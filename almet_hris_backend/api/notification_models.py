...
class EmailTemplate(models.Model):
    ...
    Supports HTML and plain text formats with variable substitution
    """
    
    TEMPLATE_TYPES = [
        # Business Trip Templates
        ('BUSINESS_TRIP_CREATED', 'Business Trip - Request Created'),
        ('BUSINESS_TRIP_LM_APPROVED', 'Business Trip - Line Manager Approved'),
        ('BUSINESS_TRIP_FINANCE_APPROVED', 'Business Trip - Finance Approved'),
        ('BUSINESS_TRIP_HR_APPROVED', 'Business Trip - HR Approved (Final)'),
        ('BUSINESS_TRIP_REJECTED', 'Business Trip - Rejected'),
        ('BUSINESS_TRIP_CANCELLED', 'Business Trip - Cancelled'),
        
        # Vacation Templates
        ('VACATION_CREATED', 'Vacation - Request Created'),
    ...
    template_type = models.CharField(
        ...
        unique=True,
        help_text="Unique template type identifier"
    )
    
    # Email content
    subject = models.CharField(
        max_length=255,
        help_text="Email subject line (supports variables like {request_id})"
    )
    body_html = models.TextField(
        help_text="HTML email body with placeholders (e.g., {employee_name}, {request_id})"
    )
    body_text = models.TextField(
        blank=True,
        help_text="Plain text email body (optional, for email clients that don't support HTML)"
    )
    
    # Template documentation
    available_variables = models.TextField(
        blank=True,
        help_text="Available variables for this template (e.g., {employee_name}, {request_id}, {start_date})"
    )
    
    # Status
    is_active = models.BooleanField(
        default=True,
        help_text="Active templates are available for use"
    )
    ...
    def render(self, context):
        ...
        Returns:
            tuple: (rendered_subject, rendered_body_html, rendered_body_text)
        """
        rendered_subject = self.subject.format(**context)
        rendered_body_html = self.body_html.format(**context)
        rendered_body_text = self.body_text.format(**context) if self.body_text else ''
        
        return rendered_subject, rendered_body_html, rendered_body_text
