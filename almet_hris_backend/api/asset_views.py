...
class AssetViewSet(viewsets.ModelViewSet):
    ...
    def dashboard_stats(self, request):
        ...
        try:
            ...
            avg_value = queryset.aggregate(avg=Avg('purchase_price'))['avg'] or 0
            ...
            # Recent activities
            recent_activities = AssetActivity.objects.select_related(
                'asset', 'performed_by'
            ...
            return Response({
                ...
                'recent_activities': AssetActivitySerializer(recent_activities, many=True).data
            })
...
class AssetAssignmentViewSet(viewsets.ModelViewSet):
    """ViewSet for Asset Assignments"""
    
    queryset = AssetAssignment.objects.select_related('asset', 'employee', 'assigned_by', 'checked_in_by').all()
    serializer_class = AssetAssignmentSerializer
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['asset', 'employee', 'check_out_date', 'check_in_date']
    search_fields = ['asset__asset_name', 'asset__serial_number', 'employee__full_name', 'employee__employee_id']
    ordering = ['-check_out_date']
    
    @action(detail=False, methods=['get'])
    def active_assignments(self, request):
        """Get all active assignments"""
        active_assignments = self.queryset.filter(check_in_date__isnull=True)
...
class AssetStatsViewSet(viewsets.ViewSet):
    """ViewSet for Asset Statistics"""
